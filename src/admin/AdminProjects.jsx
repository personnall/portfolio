  ...
  return (
    <div className="space-y-8">
      <header className="flex justify-between items-center">
        ...
        <button onClick={() => setEditing({ name: '', category: 'Frontend' })} className="cyber-button flex items-center gap-2"><Plus size={18} /> New</button>
      </header>
      ...
      {editing && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/80" onClick={() => setEditing(null)} /><form onSubmit={handleSave} className="relative w-full max-w-2xl glass-card p-8 space-y-4">
        ...
        <button disabled={isSaving} className="cyber-button w-full">{isSaving ? 'Syncing...' : 'Commit'}</button>
      </form></div>)}
    </div>
  );
}
