...
export default function AdminSkills() {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (editing.id) await supabase.from('skills').update(editing).eq('id', editing.id);
    else await supabase.from('skills').insert([editing]);
    setEditing(null);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{skills?.map(s => (
        <div key={s.id} className="glass-card p-6 flex justify-between items-center">
          ...
          <div className="flex gap-2"><button onClick={() => setEditing(s)} className="p-2 text-blue-400"><Pencil size={16} /></button><button onClick={async () => { await supabase.from('skills').delete().eq('id', s.id); queryClient.invalidateQueries(['skills']); }} className="p-2 text-pink-400"><Trash2 size={16} /></button></div>
        </div>
      ))}</div>
